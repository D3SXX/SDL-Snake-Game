...
int main(int argc, char** argv)
{
    if (argc > 1) {
        for (int i = 1; i < argc; i++) {
            ...
            if (arg == "--resolution") {
                // Ensure there is another argument after --resolution
                if (i + 1 < argc) {
                    std::string resolution = argv[i + 1];
                    size_t xPos = resolution.find('x');
                    if (xPos != std::string::npos && xPos > 0 && xPos < resolution.length() - 1) {
                        ...
                        try {
                            ...
                            break;  // Exit the loop after finding the --resolution argument
                        } catch (const std::exception& e) {
                            std::cout << "Invalid resolution values: " << e.what() << std::endl;
                        }
                    }
                }
            }
